'''
WAP using a multi-layer perceptron model in mojo taking three inputs and having at least 
3 nodes in the hidden layer for implementation of universal logical gates.
'''
...
fn main():
    ...
    for af in range(3):
        print("\nCurrently Training OR gate with", get_activation_name(af), "the activation function:")
        var network = NeuralNetwork(af)
        network.fit(X_OR, Y_OR, 0.1, 10000)

        print("\nFinal Probability for NAND gate:")
        for i in range(8):
